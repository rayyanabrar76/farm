...
export default function ChatModal({ open, onClose, farmerName, farmerInitial, produceName }: Props) {
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed bottom-0 right-0 sm:bottom-6 sm:right-6 w-full sm:w-95 h-150 bg-white z-201 rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-3">
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.from === "buyer" ? "justify-end" : "justify-start"}`}>
              ...
              <div className={`max-w-[75%] ${msg.from === "buyer" ? "items-end" : "items-start"} flex flex-col`}>
        ...
        <div className="p-4 border-t border-gray-100 bg-white">
          <div className="flex items-center gap-2 bg-gray-50 border-2 border-gray-200 focus-within:border-primary-400 rounded-2xl px-4 py-2.5 transition-colors">
            ...
            <button
              onClick={sendMessage}
              disabled={!input.trim()}
              className="w-8 h-8 rounded-xl bg-primary-500 hover:bg-primary-600 disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center transition-colors shrink-0"
            >
